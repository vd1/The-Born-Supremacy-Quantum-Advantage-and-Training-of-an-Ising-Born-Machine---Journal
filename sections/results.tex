...
\begin{align}
    \kappa_G(\mathbf{x}, \mathbf{y}) \coloneqq  \frac{1}{c}\sum_{i=1}^c\exp\left(-\frac{||\mathbf{x}-\mathbf{y}||^2_2}{2\sigma_i}\right) \label{gaussiankernel}
\end{align}
...
\noindent where $p^{\pm}_{\theta_k}$ are output distributions generated by running the following auxiliary circuits\cite{mitarai_quantum_2018, schuld_evaluating_2018} \textit{for each} 
...
\begin{multline}
    \mathcal{L}_{\SH}^\epsilon(p_{\boldsymbol\theta}, \pi) \coloneqq\\
    \OT^c_\epsilon(p_{\boldsymbol\theta}, \pi) - \frac{1}{2} \OT^c_\epsilon(p_{\boldsymbol\theta}, p_{\boldsymbol\theta}) -\frac{1}{2}\OT^c_\epsilon(\pi, \pi) \label{sinkhorndivergence}
\end{multline}
\begin{multline}
...
$\OT$, when the cost ($c(\mathbf{x}, \mathbf{y})$), is chosen to be a metric on the space of $\mathcal{X}^n$.
...
\begin{align}
&\underline{\epsilon \rightarrow 0}: \nonumber\\
&\mathcal{L}_{\SH}^0(p_{\boldsymbol\theta}, \pi)\rightarrow \OT_0^c(p_{\boldsymbol\theta}, \pi)\\
&\underline{\epsilon \rightarrow \infty}:\nonumber\\
...
\noindent where $\varphi(\mathbf{x})$ is a function which depends on the optimal solutions found to the regularised $\OT$ problem in \eqref{wassersteinregularised}. See \appref{supp_matt:sinkhorn} for more details on the Sinkhorn divergence, and its gradient.
...
Firstly, we will address the computability of $\mathcal{L}_{\SH}$ and we find, due to the results of ref. \cite{genevay_sample_2018}, a somewhat `optimal' value for $\epsilon$, such that the sample complexity of $\mathcal{L}_{\SH}$ looks efficient. Specifically, the mean error between $\mathcal{L}_{\SH}$ and its approximation $\hat{\mathcal{L}}_{\SH}$ for $n$ qubits, computed using $M$ samples, scales as:
\begin{multline}
...
\begin{align}
    ...
    = \mathcal{O}\left(\frac{n}{\sqrt{M}}\log(1/\delta)^{1/2}\right) \label{sinkhornborn_samplecomplexity_choosed_MAIN}
\end{align}
...
\end{tikzpicture}
    ...
    % \end{subfigure}
    \caption{$\MMD$ [\crule[ForestGreen]{0.2cm}{0.2cm}, \crule[yellow]{0.2cm}{0.2cm}] vs.\@ Sinkhorn [\crule[blue]{0.2cm}{0.2cm}, \crule[cyan]{0.2cm}{0.2cm}] for 4 qubits comparing performance on the real QPU ({\fontfamily{cmtt}\selectfont Aspen-4-4Q-A}) vs.\@ simulated behaviour on QVM ({\fontfamily{cmtt}\selectfont Aspen-4-4Q-A-qvm}) using 500 samples and a batch size of 250, learning target data [\crule[black]{0.2cm}{0.2cm}]. (a)$\TV$ Difference between training methods with regularisation parameter $\epsilon = 0.1$ (b) Final learned probabilities, (c) $\mathcal{L}^{0.1}_{\SH}$ on QVM  [\crule[cyan]{0.2cm}{0.2cm}] vs.\@ QPU  [\crule[blue]{0.2cm}{0.2cm}]. (d) $\mathcal{L}_{\MMD}$ on QVM  [\crule[yellow]{0.2cm}{0.2cm}] vs.\@ QPU  [\crule[ForestGreen]{0.2cm}{0.2cm}]. }\label{fig:MMDvSink4_real}
\end{figure*}
...
learn a target distribution $\pi$ which has %is believed to itself 
quantum supremacy, 
by providing a quantum circuit $C$ close enough to $\pi$ 
(i.e.\@ below a threshold error in total variation),
...
The first point does not completely fit that intuition. 
For one thing, hardness is not known to hold for the required notion of additive error 
(i.e.\@ total variation distance) but only for multiplicative error. Also, even though the model is more \textit{expressive} than any classical model\cite{du_expressive_2018}, this does not imply that it could actually \textit{learn} a hard distribution. On the other hand, it is easy to see why the converse would be true, if the $\IBM$ could learn a distribution which is hard to sample from classically, the underlying circuit must have, at some point, reached a circuit configuration for which the output distribution is hard to classically sample.

We can address the first point informally (see \appref{supp_matt:hardness} for the formal statements and proof) in three steps:
\begin{itemize}
